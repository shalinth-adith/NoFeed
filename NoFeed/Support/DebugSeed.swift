...
enum DebugSeed {
    ...
    /// `NoFeedPreviewShield Instagram` — stand screen 03 up as the app's root.
    ///
    ...
    static var shieldPreviewSubject: String? {
        argument(after: "NoFeedPreviewShield")
    }

    /// `NoFeedPreviewBlockScreen Instagram` — stand screen 03 (the block screen)
    ...
    static var blockScreenPreviewSubject: String? {
        argument(after: "NoFeedPreviewBlockScreen")
    }
    ...
    /// `NoFeedSeedDemoHistory` — fill this calendar week with focus sessions so
    /// Insights renders its populated state instead of its first-run state.
    static var wantsDemoHistory: Bool {
        ProcessInfo.processInfo.arguments.contains("NoFeedSeedDemoHistory")
    }

    /// `NoFeedSeedManyProfiles` — add enough profiles that Home's switcher row
    ...
    static var wantsManyProfiles: Bool {
        ProcessInfo.processInfo.arguments.contains("NoFeedSeedManyProfiles")
    }
    ...
    static func clearSessionIfRequested() {
        guard ProcessInfo.processInfo.arguments.contains("NoFeedUITestResetSession")
        else { return }
