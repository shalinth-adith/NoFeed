...
//
//  Imported from the Claude Design spec (NoFeed Quiet.dc.html — the Begin focus /
//  Done CTA: background:var(--tone); color:#0A0B0E; radius 16; weight 600; flat).
//

import SwiftUI

struct NoFeedPrimaryButtonStyle: ButtonStyle {
...
struct NoFeedSecondaryButtonStyle: ButtonStyle {
...
extension ButtonStyle where Self == NoFeedPrimaryButtonStyle {
    static var zenlyPrimary: NoFeedPrimaryButtonStyle { NoFeedPrimaryButtonStyle() }
    static func zenlyPrimary(tint: Color, height: CGFloat = 56) -> NoFeedPrimaryButtonStyle {
        NoFeedPrimaryButtonStyle(height: height, tint: tint)
    }
}

extension ButtonStyle where Self == NoFeedSecondaryButtonStyle {
    static var zenlySecondary: NoFeedSecondaryButtonStyle { NoFeedSecondaryButtonStyle() }
}
