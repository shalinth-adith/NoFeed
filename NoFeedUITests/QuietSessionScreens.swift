...
//  grant, so the suite passes `NoFeedUITestBypassScreenTime` (a DEBUG-only flag
//  in AuthorizationService). Screen 03 is not a SwiftUI view at all — iOS draws
//  it inside the NoFeedShield extension — so it is captured through the DEBUG
...
final class QuietSessionScreens: XCTestCase {
    ...
    private func launchToHome(_ extra: [String] = []) -> XCUIApplication {
        let app = launch(["NoFeedUITestBypassScreenTime"] + extra)
    ...
    func testAppPausedScreen() {
        let app = launch(["NoFeedPreviewBlockScreen", "Instagram"])
    ...
    func testBackToFocusScreen() {
        let app = launch(["NoFeedPreviewShield", "Instagram"])
    ...
    func testInsightsPopulatedScreen() {
        let app = launchToHome(["NoFeedSeedDemoHistory"])
        app.tabBars.buttons["Insights"].tap()
